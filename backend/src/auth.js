...
async function findUser(userId) {
  return mem.findUser(userId);
}

// Helper to find a session in memory or fallback to DB
async function findSession(userId, jti) {
  return mem.findSession(userId, jti);
}
...
export async function requireAuth(req, res, next) {
  try {
    ...
    const sess = await findSession(user.id);
